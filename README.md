...
nohup python3 flux_vast_min_server.py --host 0.0.0.0 --port 8910 --offload none --preload-model flux2-dev-bnb-4bit --max-batch-size 4 --batch-wait-ms 1000 > /workspace/logs/flux.log 2>&1 &
```
...
curl -X POST http://HOST:8910/batch/config \
  -H 'Content-Type: application/json' \
  -d '{"max_batch_size":4, "batch_wait_ms":1000}'
```
