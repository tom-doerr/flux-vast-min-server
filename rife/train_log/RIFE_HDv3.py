...
from rife.train_log.IFNet_HDv3 import *
import torch.nn.functional as F
# vendored: training-only losses removed (inference path unused)
...
class Model:
    def __init__(self, local_rank=-1):
        ...
        self.epe = None  # vendored: training-only (loss module removed)
        self.version = 4.25
        # self.vgg = VGGPerceptualLoss().to(device)
        self.sobel = None  # vendored: training-only (loss module removed)
        if local_rank != -1:
